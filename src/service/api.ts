...
import {IByUserId, IPublishNews, IUpdateSend} from "../store/news/types";
...
export const get_current_user = (token: string) => _api.get('/auth/me', {headers: {Authorization: token}}) //auth
...
export const get_news_by_user_id = ({userId, token, page}: IByUserId) =>
    _api.get(`/news/user_by_news/${userId}?page=${page}`, {headers: {Authorization: token}}) //auth
